...
* 集群必须运行ONTAP 9.11.1或更高版本(对于Azure、为9.12.1)
...
* 激活Cloud Backup时选择的DataLock选项必须用于该集群的所有备份策略。
...
+
如果您的内部集群使用的是ONTAP 9.12.1或更高版本、您可以选择在一定天数后将旧备份分层到云备份UI中的_Archive_存储、以进一步优化成本。 link:reference-google-backup-tiers.html["了解有关Google归档存储的更多信息"^]。
