...
+
--
* 在Cloud Manager中添加目标AWS帐户凭据
* 将权限"S3：PutBucketPolicy"添加到为Cloud Manager提供权限的IAM角色中
...
需要 AWS 权限:: 为 Cloud Manager 提供权限的 IAM 角色必须包含最新版本的 S3 权限 https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html["Cloud Manager 策略"^]。
+
--
以下是策略中的特定权限：
...
+
如果要使用其他AWS帐户进行备份、则必须在Cloud Manager中添加目标AWS帐户凭据、并将权限"S3：PutBucketPolicy"添加到为Cloud Manager提供权限的IAM角色。
