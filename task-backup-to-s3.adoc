...
* 为BlueXP Connector提供权限的IAM角色包括最新版本的S3权限 https://mysupport.netapp.com/site/info/cloud-manager-policies["BlueXP策略"^]。
...
--
* 在BlueXP中添加目标AWS帐户凭据
* 将权限"S3：PutBucketPolicy"添加到为BlueXP提供权限的IAM角色中
...
+
如果要使用其他AWS帐户进行备份、则必须在BlueXP中添加目标AWS帐户凭据、并将权限"S3：PutBucketPolicy"添加到为BlueXP提供权限的IAM角色。
