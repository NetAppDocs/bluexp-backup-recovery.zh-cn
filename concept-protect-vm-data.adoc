...
[role="lead"]
您可以通过将数据存储库从内部主存储和二级存储备份到云来保护虚拟机上的数据。如果需要、您可以将虚拟机还原回适用于VMware vSphere的内部部署SnapCenter 插件主机。

您可以将数据存储库备份到Amazon Web Services S3、Microsoft Azure Blob、Google Cloud Platform和StorageGRID。
