...
以前、必须在启动Snapshot、复制或备份之前创建自定义策略。从ONTAP 9.8 P5开始、您现在可以使用BlueXP备份和恢复UI在备份激活过程中创建策略。
