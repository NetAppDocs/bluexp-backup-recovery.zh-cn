...
. 复制位于以下位置的* sudoer.txt *的内容：_/var/lib/doker/volumes/service-manager[1]-2_Cloud manager_SCS云卷/_data/$(sudo Docker ps|gep -po"Cloud manager_SCS云：.*？"|sed -e 's/*$//'|Cut -f2 -d：")/SC-Linux-host-plugin_
