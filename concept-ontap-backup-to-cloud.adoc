...
* 内部ONTAP 系统支持完整卷还原；目前不支持Cloud Volumes ONTAP 系统。
* 内部ONTAP 和Cloud Volumes ONTAP 系统均支持文件级还原。
* 当前不支持目录/文件夹还原。
* 无法将备份移动到归档存储。
* 备份无法使用DataLock和勒索软件保护。
...
* 目前、对象存储上的WORM/Compliance模式仅在Amazon S3和StorageGRID 上受支持。这称为DataLock功能、必须使用Cloud Backup设置对其进行管理。
...
* 如果在不同子网中对不同的BlueXP系统使用同一帐户、则不支持文件级还原。
