...
Cloud Backup可在没有Internet连接的站点(也称为"私有模式"或"暗"站点)中使用、以备份卷数据。此配置还支持卷和文件还原。在这种情况下、您需要在非公开站点中部署BlueXP Connector。

* 您可以将本地内部ONTAP 系统中的数据备份到本地NetApp StorageGRID 系统。请参见 link:task-backup-onprem-private-cloud.html["将内部 ONTAP 数据备份到 StorageGRID"] 了解详细信息。
* 您可以将安装在AWS商业区域和AWS C2S/SC2S安全区域的Cloud Volumes ONTAP 系统中的数据备份到Amazon S3。请参见 link:task-backup-to-s3.html["将 Cloud Volumes ONTAP 数据备份到 Amazon S3"]。
* 您可以将安装在Azure商业区域和Azure IL6安全区域中的Cloud Volumes ONTAP 系统中的数据备份到Azure Blob。请参见 link:task-backup-to-azure.html["将Cloud Volumes ONTAP 数据备份到Azure Blob"]。
...
Cloud Backup可在互联网连接有限的站点(也称为"受限模式")中使用、以备份卷数据。此配置还支持卷和文件还原。在这种情况下、您需要在受限区域部署BlueXP Connector。
...
您可以选择对所有卷进行每小时、每天、每周、每月和每年备份的组合。应用于卷的Snapshot策略必须是Cloud Backup可识别的策略之一。您还可以选择系统定义的策略之一，这些策略可提供 3 个月， 1 年和 7 年的备份和保留期限。这些策略包括：
...
+
如果集群使用的是ONTAP 9.12.1或更高版本、您可以选择在一定天数后在Cloud Backup UI中将较早的备份分层到_Archive_存储、以进一步优化成本。 link:reference-google-backup-tiers.html["了解有关Google归档存储的更多信息"^]。
...
* 目前、Amazon S3、Azure和StorageGRID 支持对象存储上的WORM/Compliance模式。这称为DataLock功能、必须使用Cloud Backup设置来管理、而不是使用云提供商界面。
