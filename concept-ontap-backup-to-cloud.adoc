...
Cloud Backup可在互联网连接有限的站点(也称为"受限模式")中使用、以备份和还原卷数据。在这种情况下、您需要在受限区域部署BlueXP Connector。
...
Cloud Backup可在没有Internet连接的站点(也称为"私有模式"或"暗"站点)中使用、以备份卷数据。在这种情况下、您需要在非公开站点中部署BlueXP Connector。
...
您可以选择对所有卷进行每小时、每天、每周、每月和每年备份的组合。应用于卷的Snapshot策略必须是Cloud Backup可识别的策略之一、否则不会创建备份文件。您还可以选择系统定义的策略之一，这些策略可提供 3 个月， 1 年和 7 年的备份和保留期限。这些策略包括：
