...
[role="lead"]
使用 BlueXP 备份和恢复，监控您启动的本地快照、复制和备份到对象存储作业的状态，以及您启动的恢复作业。您可以查看已完成、正在进行或失败的作业、以便诊断和修复问题。使用BlueXP通知中心、您可以启用通过电子邮件发送通知、以便即使您未登录系统、也可以获得重要系统活动的通知。使用BlueXP时间线、您可以查看通过UI或API启动的所有操作的详细信息。
