...
. 选择*作业监控*选项卡。
+
image:screenshot_backup_job_monitor2.png["显示作业监控信息板中备份和还原作业的屏幕截图。"]
...
您可以使用多个筛选器筛选"作业监控"页面上的操作、例如策略、Snapshot标签、操作类型(保护、还原、保留或其他)、卷上的自定义标记以及保护类型(本地Snapshot、复制或备份到云)。
