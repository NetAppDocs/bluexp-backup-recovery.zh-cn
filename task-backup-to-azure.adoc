...
默认情况下、Cloud Backup会为Blob容器配置本地冗余(LRS)以优化成本。如果要确保在不同分区之间复制数据、您可以在激活Cloud Backup后将此设置更改为区域冗余(ZRS)。请参见的Microsoft说明 https://learn.microsoft.com/en-us/azure/storage/common/redundancy-migration?tabs=portal["更改存储帐户的复制方式"^]。
