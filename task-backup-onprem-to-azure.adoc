...
如果Connector部署在云中、则Azure政府区域支持Cloud Backup、而不是安装在内部或非公开站点中。此外、您还必须从Azure Marketplace部署Connector。您不能从Cloud Manager SaaS网站在政府区域部署Connector。 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-launching-azure-mktp.html["了解如何在Azure政府区域创建Connector"]。
