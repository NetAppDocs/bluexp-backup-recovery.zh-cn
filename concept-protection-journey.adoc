...
您可以在BlueXP备份和恢复UI中创建Snapshot、复制和备份到对象存储策略。请参见一节中的 link:task-manage-backups-ontap.html#add-a-new-backup-policy["正在添加新备份策略"] 了解详细信息。
...
如果您已经在使用BlueXP管理存储、则表示已安装BlueXP Connector。如果您计划将同一个Connector与BlueXP备份和恢复结合使用、则一切都准备就绪。如果您需要使用其他Connector、则需要在开始备份和恢复实施之前对其进行部署。
...
在标准模式下完全支持BlueXP备份和恢复—站点具有完全Internet连接。在这种情况下、您可以在由BlueXP管理的任何内部ONTAP或Cloud Volumes ONTAP系统上创建复制的卷、并且可以在任何受支持的云提供商的对象存储上创建备份文件。 link:concept-ontap-backup-to-cloud.html#supported-backup-destinations["查看受支持备份目标的完整列表"]。

有关有效连接器位置的列表、请参见计划在其中创建备份文件的云提供程序的备份主题。当Connector部署在特定云提供程序中或手动安装在Linux计算机上时、可能存在一些限制。
...
BlueXP备份和恢复在受限模式下受支持- Internet连接受限的站点。在这种情况下、您只能为有限的一组云提供商创建备份文件。您需要在受限区域部署BlueXP Connector。
...
在专用模式下支持BlueXP备份和恢复—站点没有Internet连接。在这种情况下、您只能为一组有限的对象存储提供程序创建备份文件。您需要在同一内部站点的Linux主机上部署BlueXP Connector。
