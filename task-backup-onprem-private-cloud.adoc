...
[role="quick-margin-para"]
如果您已经在StorageGRID或内部部署了连接器、则一切都准备就绪。如果没有、则需要创建BlueXP连接器、将ONTAP数据备份到StorageGRID。您还需要自定义连接器的网络设置、以便它可以连接到StorageGRID。
...
* 使用扇出备份架构时、必须在主存储系统上配置以下设置。
* 使用级联备份架构时、必须在二级存储系统上配置以下设置。
