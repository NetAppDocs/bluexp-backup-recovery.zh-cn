...
StorageGRID 必须满足以下要求。请参见 https://["StorageGRID 文档"^] 有关详细信息 ...
...
--
S3 凭据:: 您必须已创建S3租户帐户来控制对StorageGRID 存储的访问。 https://["有关详细信息、请参见StorageGRID 文档"^]。
+
--
...
* https://["了解连接器"^]
* https://["在可访问 Internet 的 Linux 主机上安装 Connector"^]
* https://["在无法访问 Internet 的 Linux 主机上安装 Connector"^]
* https://["在连接器之间切换"^]



NOTE: BlueXP备份和恢复功能内置在BlueXP Connector中。如果安装在无法连接 Internet 的站点上，则需要定期更新 Connector 软件才能访问新功能。检查 link:whats-new.html["BlueXP备份和恢复新增功能"] 要查看每个BlueXP备份和恢复版本中的新功能、然后您可以按照中的步骤进行操作 https://["升级 Connector 软件"^] 希望使用新功能时。
...
* 您的StorageGRID 必须是 https://["已在BlueXP画布中发现并提供"^]。
...
您选择的归档策略设置将在StorageGRID 中生成信息生命周期管理(ILM)策略、并将这些设置添加为"规则"。如果存在活动的ILM策略、则新规则将添加到ILM策略中、以将数据移动到归档层。如果现有ILM策略处于"建议"状态、则无法创建和激活新的ILM策略。 https://["详细了解StorageGRID ILM策略和规则"^]。
