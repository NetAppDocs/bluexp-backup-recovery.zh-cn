...
[role="lead"]
BlueXP backup and recovery服务为ONTAP卷、Microsoft SQL Server 实例和数据库、VMware 工作负载（预览版）和 Kubernetes 工作负载（预览版）提供高效、安全且经济高效的数据保护。


NOTE: 有关保护 VMware 和 Kubernetes 工作负载的文档以技术预览的形式提供。对于此预览版产品， NetApp保留在正式发布之前修改产品详细信息、内容和时间表的权利。
...
** 无需SnapCenter即可使用BlueXP backup and recovery。
...
+
** 使用BlueXP backup and recovery保护您的 VMware VM 和数据存储。
** 将 VMware 工作负载备份到 Amazon Web Services S3 或StorageGRID （预览版）。
** 将 VMware 数据从云端恢复到本地 vCenter。
** 无需SnapCenter Plug-in for VMware vSphere即可使用BlueXP backup and recovery。
...
** 使用BlueXP backup and recovery将虚拟机数据从云端恢复到本地 vCenter。您可以将虚拟机恢复到与备份完全相同的位置或其他位置。
** 使用BlueXP backup and recovery以及适用于SnapCenter Plug-in for VMware vSphere。
...
* *工作负载*： BlueXP backup and recovery中的工作负载可以包括ONTAP卷、Microsoft SQL Server 实例和数据库；VMware VM 和数据存储区；或 Kubernetes 集群和应用程序。
