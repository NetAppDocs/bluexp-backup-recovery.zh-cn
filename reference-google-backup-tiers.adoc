...
如果您的内部集群使用的是ONTAP 9.12.1或更高版本、您可以选择在一定天数(通常超过30天)后在云备份UI中将较早的备份分层到_Archive_存储、以进一步优化成本。(此功能当前不适用于Cloud Volumes ONTAP 系统。)此层中的数据需要较高的检索成本、因此您需要考虑从这些归档备份文件还原数据的频率。请参见关于一节 <<从归档存储还原数据,从归档存储还原数据>>。
