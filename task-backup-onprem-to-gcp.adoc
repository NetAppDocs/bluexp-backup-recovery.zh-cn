...
+
如果您使用的是专用Google访问或专用服务连接、请确保已将DNS服务器配置为将storage.googleapis.com指向正确的内部(私有) IP地址。
...
+
按照Google说明设置这些私有访问选项。确保DNS服务器已配置为将www.googleapis.com和storage.googleapis.com指向正确的内部(私有) IP地址。
