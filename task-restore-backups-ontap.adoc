...
使用ONTAP 9.13.0或更高版本将云备份还原到Cloud Volumes ONTAP系统时、您可以选择执行"快速还原"操作。快速还原非常适合需要尽快提供对卷的访问权限的灾难恢复情形。快速还原会将元数据从备份文件还原到卷、而不是还原整个备份文件。不建议对性能或延迟敏感型应用程序执行快速还原、归档存储中的备份也不支持快速还原。
...
+
(可选)搜索自定义标记、以便仅列出具有特定标记的卷。

+
image:screenshot_restore_select_volume_snapshot2.png["选择要还原的工作环境，卷和卷备份文件的屏幕截图。"]
...
+
使用ONTAP 9.13.0或更高版本将备份从对象存储还原到Cloud Volumes ONTAP系统时、您可以选择执行"快速还原"操作。
...
* 只有在创建云备份的源系统运行的是ONTAP 9.12.1或更高版本时、FlexGroup卷才支持快速还原。只有在创建云备份的源系统运行的是ONTAP 9.11.0或更高版本时、才支持SnapLock卷快速还原。FlexVol卷没有最低要求。
