...
summary: '在无法访问Internet的站点中使用BlueXP备份和恢复时(称为"专用模式")、BlueXP备份和恢复配置数据将备份到存储备份的StorageGRID存储分段。如果将来您的问题描述与BlueXP Connector主机系统结合使用、则可以部署新的Connector并还原关键的BlueXP备份和恢复数据。' 
---
...
[role="lead"]
在无法访问Internet的站点(称为"专用模式"或"非公开站点")中使用BlueXP备份和恢复时、BlueXP备份和恢复配置数据会备份到存储备份的StorageGRID存储分段。如果将来您的问题描述与BlueXP Connector主机系统结合使用、则可以部署新的Connector并还原关键的BlueXP备份和恢复数据。
...
.您需要的内容
您需要从存储备份文件的StorageGRID存储分段访问最新的数据库和索引备份：
