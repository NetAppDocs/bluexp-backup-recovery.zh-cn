...
[role="lead"]
在无法访问Internet的站点中使用BlueXP备份和恢复时(称为"专用模式")、BlueXP备份和恢复配置数据将备份到存储备份的StorageGRID或ONTAP S3存储分段。如果将来您的问题描述与BlueXP Connector主机系统结合使用、则可以部署新的Connector并还原关键的BlueXP备份和恢复数据。
...
此数据每天午夜备份一次、每个文件最多保留30个副本。如果Connector管理多个内部ONTAP工作环境、BlueXP备份和恢复文件将位于首先激活的工作环境的存储分段中。
