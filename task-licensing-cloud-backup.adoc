...
* "CVO Edge Cache"软件包的功能与"CVO专业版"软件包相同、但也支持 https://docs.netapp.com/us-en/cloud-manager-file-cache/concept-gfc.html["全局文件缓存"^]。您有权为Cloud Volumes ONTAP 系统上配置的每个3 TiB容量部署一个全局文件缓存边缘系统。此选项可通过Azure和Google MarketPlaces使用、但不支持备份内部ONTAP 数据。
